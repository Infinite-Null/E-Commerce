...
export function ReviewsComponent({List,OnYesPress,OnSearchPress}) {
    ...
    return (
           <>
               <Input variant={"flat"} size={"lg"} label={"Enter Product Id"} className={"w-72 mt-10 border-1 border-black"} style={{
                   fontSize:"30px",
               }} onChange={(e)=>{
               ...
               <Card className={"m-5 border-black border-1 text-lg w-[95vw] no-underline"} isHoverable={true}>
                   ...
                   {List.map((e,i)=><EachComponent Rating={e.Rating} ReviewId={e.ReviewId} User={e.User} OnYesPress={OnYesPress} Comment={e.Comment} key={i}/>)}
               </Card>
           </>
    )
}
